...
private:
    ...
    float *Adresse_Pile;
public:
    C_PILE_ENTIER(const C_PILE_ENTIER &P_Tab);
    C_PILE_ENTIER(int);
    C_PILE_ENTIER();
    ~C_PILE_ENTIER();
    void Empile(int);
...
C_PILE_ENTIER::C_PILE_ENTIER(const C_PILE_ENTIER &P_pile) {
    
    Nombre_max = P_pile.Nombre_max;
    Adresse_Pile = new float[Nombre_max];
    for (int i = 0; i < Nombre_max; i++) {
        Adresse_Pile[i] = P_pile.Adresse_Pile[i];
    }
    Nombre_Element=P_pile.Nombre_Element;
}
